...
export const VendorCustomerList = ({ vendorId }: { vendorId: string }) => {
  ...
  const getBadge = (c: Customer) => {
    const totalEngagement = (c.inquiry_count || 0) + (c.message_count || 0);
    if (totalEngagement >= 5) return <Badge className="bg-primary/20 text-primary border-primary/30">Highly Engaged</Badge>;
    if (totalEngagement >= 2) return <Badge className="bg-primary/10 text-primary/80 border-primary/20">Repeat visitor</Badge>;
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm font-semibold">{c.inquiry_count || 0} inquiries</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <Edit3 className="h-3.5 w-3.5 text-muted-foreground" />
                          <span className="text-sm font-semibold">{c.message_count || 0} messages</span>
                        </div>
                      </div>
                    </div>
                  </div>
