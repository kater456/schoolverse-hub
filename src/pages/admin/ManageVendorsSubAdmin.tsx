...
import { Check, X, Eye, Loader2, FileText, ShieldCheck, Globe, AlertTriangle, Ban } from "lucide-react";
...
const ManageVendorsSubAdmin = () => {
  ...
  const fetchVendors = async () => {
    ...
    let query = supabase
      .from("vendors")
      .select(`*, schools(name), campus_locations(name), vendor_images(id, image_url, is_primary), vendor_private_details(*), vendor_ratings(rating), vendor_comments(id)`)
      .order("created_at", { ascending: false });

    if (assignedSchoolId) {
      query = query.eq("school_id", assignedSchoolId);
    }
  ...
  useEffect(() => { fetchVendors(); }, [assignedSchoolId]);
  ...
  const removeVendor = async (id: string) => {
    ...
    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); }
    else { toast({ title: "Vendor rejected" }); fetchVendors(); }
  };

  const toggleVerified = async (id: string, current: boolean) => {
    const { error } = await supabase.from("vendors").update({ is_verified: !current } as any).eq("id", id);
    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); }
    else { toast({ title: !current ? "Vendor verified" : "Verification removed" }); fetchVendors(); }
  };
  ...
  const getFlags = (v: any) => {
    ...
    if (poorReviews >= POOR_REVIEW_THRESHOLD) {
      flags.push({ label: `${poorReviews} poor reviews`, color: "bg-destructive/20 text-destructive", icon: "alert" });
    }
    if (v.is_active === false) {
      flags.push({ label: "Inactive", color: "bg-orange-500/20 text-orange-600", icon: "ban" });
    }
    return flags;
  };

  const renderTable = (list: any[], showRestore = false) => (
    <Table>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
          ...
          return (
            <TableRow key={v.id}>
                ...
                <div className="flex flex-wrap gap-1">
                  {flags.map((f, i) => (
                    <Badge key={i} className={`text-[10px] ${f.color}`}>
                      {f.icon === "alert" && <AlertTriangle className="h-3 w-3 mr-0.5" />}
                      {f.icon === "ban" && <Ban className="h-3 w-3 mr-0.5" />}
                      {f.label}
                    </Badge>
                  ))}
                </div>
              ...
              <TableCell className="text-right space-x-1">
                <Button size="icon" variant="ghost" onClick={() => setSelectedVendor(v)}><Eye className="h-4 w-4" /></Button>
                {!showRestore && !v.is_approved && v.is_active && (
                  <Button size="icon" variant="ghost" className="text-success" onClick={() => approveVendor(v.id)}><Check className="h-4 w-4" /></Button>
                )}
                {!showRestore && v.is_active && pd?.id_document_url && (
                  <Button size="icon" variant="ghost" className="text-primary" onClick={() => toggleVerified(v.id, !!v.is_verified)}>
                    <ShieldCheck className="h-4 w-4" />
                  </Button>
                )}
                {!showRestore && v.is_active && (
                  <Button size="icon" variant="ghost" className="text-destructive" onClick={() => removeVendor(v.id)}><X className="h-4 w-4" /></Button>
                )}
              </TableCell>
            </TableRow>
          );
        ...
        {list.length === 0 && (
          <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No vendors</TableCell></TableRow>
        )}
      </TableBody>
    </Table>
  );
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="active">
        ...
        <TabsContent value="active">
          <Card className="border-border/50"><CardContent className="p-0">{renderTable(activeVendors)}</CardContent></Card>
        </TabsContent>
        <TabsContent value="rejected">
          <Card className="border-destructive/30">
            <CardHeader><CardTitle className="text-base text-destructive flex items-center gap-2"><Ban className="h-4 w-4" /> Rejected Vendors</CardTitle></CardHeader>
            <CardContent className="p-0">{renderTable(rejectedVendors, true)}</CardContent>
          </Card>
        </TabsContent>
      </Tabs>
            ...
            <div className="space-y-4">
              ...
              {privateDetails && (
                <div className="border-t pt-4">
                  <h4 className="font-semibold text-sm mb-1 text-destructive">🔒 Private Info</h4>
                  <div className="text-sm space-y-1 text-muted-foreground">
                    ...
                    {privateDetails.id_document_url && (
                      <div className="mt-2">
                        ...
                        <a href={privateDetails.id_document_url} target="_blank" rel="noopener noreferrer" className="text-primary underline text-xs">View ID Document</a>
                      </div>
                    )}
                  </div>
                </div>
